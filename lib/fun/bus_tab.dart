...
class _BusTabState extends State<BusTab> with SingleTickerProviderStateMixin {
  ...
  List<NLatLng> _realRouteCoords = []; // 실제 도로 경로 좌표
  ...
  Future<void> _addRoutePath() async {
    // 실제 도로 경로가 있으면 사용, 없으면 정류장 직선 연결
    final coords = _realRouteCoords.isNotEmpty 
        ? _realRouteCoords 
        : _selectedRouteStops.map((s) => NLatLng(s.lat, s.lon)).toList();
  ...
  /// 실제 도로를 따라가는 노선 경로 생성
  Future<void> _generateRealRoutePath() async {
    if (_selectedRouteStops.isEmpty) return;
    
    print('🛣️ 노선 실제 도로 경로 생성 시작...');
    print('🔍 선택된 정류장 수: ${_selectedRouteStops.length}');
    
    // ODsay API 테스트 (경산 좌표 사용)
    print('🧪 ODsay API 테스트 시작...');
    try {
      final testResult = await _apiService.getRouteCoordinates(
        startLat: 35.8262,  // 경산 좌표
        startLon: 128.7513,
        endLat: 35.8345,   // 경산 근처 좌표  
        endLon: 128.7621,
      );
      print('🧪 ODsay 테스트 결과: ${testResult.length}개 좌표');
    } catch (e) {
      print('🧪 ODsay 테스트 실패: $e');
    }
    
    try {
      // ODsay API로 실제 경로 생성 (정류장이 많으면 일부만)
      final routeStops = _selectedRouteStops.length > 10 
          ? _selectedRouteStops.take(10).toList() // 처음 10개만 사용하여 API 호출 제한
          : _selectedRouteStops;

      final realPathCoords = await _apiService.getBusRouteRealPath(routeStops);
      
      setState(() {
        _realRouteCoords = realPathCoords.map((coord) => 
          NLatLng(coord['lat']!, coord['lng']!)
        ).toList();
      });
      
      print('✅ 노선 실제 도로 경로 생성 완료: ${_realRouteCoords.length}개 좌표');
    } catch (e) {
      print('⚠️ 실제 경로 생성 실패, 직선 경로 사용: $e');
      // 실패 시 기존 직선 경로 사용
      setState(() {
        _realRouteCoords = _selectedRouteStops.map((stop) => 
