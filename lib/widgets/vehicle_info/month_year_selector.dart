import 'package:flutter/material.dart';
import '../../utils/constants.dart';

/// 년월 선택기 위젯
class MonthYearSelector extends StatelessWidget {
  final int year; // 선택된 년도
  final int month; // 선택된 월
  final VoidCallback onMonthYearPressed; // 년월 선택 버튼 클릭 콜백

  const MonthYearSelector({
    super.key,
    required this.year,
    required this.month,
    required this.onMonthYearPressed,
  });

  @override
  Widget build(BuildContext context) {
    // 테마 색상 스키마 가져오기
    final colorScheme = Theme.of(context).colorScheme;

    // 년월 포맷팅 (yyyy년 MM월 형식)
    final yearMonthStr = '$year년 ${month.toString().padLeft(2, '0')}월';

    // 카드 형태로 감싸서 이미지와 동일한 깊이감/모서리 구현
    return Container(
      decoration: BoxDecoration(
        color: colorScheme.surface,
        borderRadius: BorderRadius.circular(12),
        boxShadow: [
          BoxShadow(
            color: colorScheme.scrim.withValues(alpha: 0.12),
            blurRadius: 14,
            offset: const Offset(0, 6),
          ),
        ],
      ),
      child: InkWell(
        onTap: onMonthYearPressed,
        borderRadius: BorderRadius.circular(12),
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 16),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              // 년월 텍스트
              Text(
                yearMonthStr,
                style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
                  fontFamily: AppConstants.fontFamilyBig,
                  color: colorScheme.onSurface,
                ),
              ),
              const SizedBox(width: 8),
              // 화살표 아이콘
              Icon(
                Icons.arrow_drop_down,
                size: 28,
                color: colorScheme.onSurface.withValues(alpha: 0.6),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
