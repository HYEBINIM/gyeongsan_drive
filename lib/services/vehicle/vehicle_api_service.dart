...
class VehicleApiService {
  ...
  Future<VehicleData?> getRealTimeCarInfo({
    ...
    try {
      ...
      final payload = await _callGetCarInfoWithFreshReconnect(
        mtId: normalizedMtId,
  ...
  Future<Map<String, dynamic>> _callGetCarInfoWithFreshReconnect({
    required String mtId,
    required int interval,
  }) async {
    const toolName = 'get_car_info';
    final primaryArguments = <String, dynamic>{
      'mt_id': mtId,
    ...
    try {
      ...
      if (!_shouldRetryWithFreshConnection(e.message)) {
        rethrow;
      }

      AppLogger.warning(
        '🚗 [차량조회] get_car_info 세션 재연결 후 재시도: '
        'mtId="$mtId", interval="$interval", error=$e',
      );

      _mcpClient.resetConnection();
      return _mcpClient.callTool(toolName, primaryArguments);
    }
  }

  bool _shouldRetryWithFreshConnection(String message) {
    final normalized = message.toLowerCase();
    return normalized.contains('invalid request parameters') ||
        normalized.contains('-32602') ||
        normalized.contains('status_code=400') ||
        normalized.contains('api 호출 실패: 400');
  }
